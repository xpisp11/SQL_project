/* Propojení tabulek economies, countries, religions - uloeno jako v_joined_eco_co_rel */

-- Oprava chybnıch dat v tabulce religions u Afghánistánu:
UPDATE religions 
...
-- Pøíprava sloupcù s poètem pøíslušníkù jednotlivıch náboenství:
...
/* Propojení covid19_basic_differences s lookup_table a covid19_tests - uloeno jako v_joined_cov_lt_tests_eco_co_rel  */

-- Vytvoøení tabulky covid19_test_new uloené ve VIEW:
...
/* Kvùli problémùm se dvìma záznami o testech u jednoho data u nìkterıch zemí (viz. Projekt_priprava_kontrolni.sql) byla vytvoøena nová tabulka 
   covid19_tests_new (viz. Projekt_upravene_tabulky.sql), která je pouita místo tabulky covid19_tests. */
...
-- Spojení dvou novì vytvoøenıch tabulek dohromady:
SELECT 
	base.`date` AS datum,
	...
	CONCAT(ROUND(v.`jiná_náboenství`/base.population * 100,1), ' %') AS "podíl_pøíslušníkù_jinıch_náb."	
...
WITH 
weather_new AS		-- vytvoøení nové tabulky se zmìnìnımi názvy mìst a provedenımi vıpoèty
(
	SELECT		
		CAST(`date` AS date) AS datum,
		-- Vıpoèty pro poadované údaje z tabulky weather ve sloupcích s teplotou, vìtrem a deštìm:
...
(
	SELECT 
		...
		w.*
	FROM countries c 
    INNER JOIN weather_new w		-- Brno a Stornoway nejsou v tabulce countries, take jejich ISO je NULL a ve vısledné tabulce nejsou tøeba, proto INNER JOIN.
...
/* Napojení údajù k life_expectancy  - uloeno jako v_Petra_Rohlickova_projekt_SQL_final*/
